(function() {
	cola(function(model) {
		...
		showPageDomTree = function(url) {
			$.ajax("./service/frame/component/load", {
				data : {
					"url" : url
				}
		...
		model.dataType({
			...
			/*
			 * , roles:{ dataType: "Role", provider: {
			 * url:"./service/frame/role/page/?companyId={{@id}}", pageSize:
			 * 10, beforeSend: function(self, arg) { data = arg.options.data;
			 * return data; } } }
			 */
			}
		});
		...
		model.action({
					...
					ok : function() {
						...
						if (validate) {
							...
							data.companyId = model.get("companyId");
					...
					usersForSelectSave : function() {
						var users = editItem = model.get("currentRole").get(
								"usersForSelect");
						var memberIds = [];
						if (!users) {
							return;
						...
						var roleId = editItem = model.get("currentRole").get(
								"id");
						...
						cola.confirm("您确定要修改角色资源吗？", {
							onApprove : function() {
								...
								return $.ajax("./service/frame/role/urls/", {
									...
									type : "POST",
									beforeSend : function() {
										cola.widget("saveRoleUrlsButton").set(
												"disabled", true);
										cola.widget("saveRoleUrlsButton")
												.addClass("loading");
									},
									success : function() {
										...
										cola.widget("resourcesLayer").hide();
										notifySucessMsg();
									},
									complete : function() {
										return NProgress.done();
					...
					saveRoleDepts : function() {
						cola.confirm("您确定要修改角色成员(部门)吗？", {
							onApprove : function() {
								...
								var delUrlIds = [];
								getSelectNodes(nodes, ids, delUrlIds);
								data.memberIds = ids;
								return $.ajax("./service/frame/role/members/",
										{
											data : JSON.stringify(data),
					...
					refreshUrlAuthCache : function() {
						cola.confirm("您确定要更新权限缓存吗？", {
							onApprove : function() {
								var url = "./service/frame/role/refresh/"
										+ new Date().getTime() + "/";
								return $.ajax(url, {
									type : "GET",
									success : function() {
					...
					switchPageDomTreeView : function(self, arg) {
						...
						// handler(el);
						// }
						// return (ref = el.get("nodes")) != null ?
						// ref.each(findComponent) : void 0;
						// };
					...
					saveRoleComponents : function() {
						...
						index = cola.widget("cardBookDataView").get(
								'currentIndex');
						list = [];
						...
						return $.ajax("./service/frame/role/urlcomps/", {
							...
							type : "POST",
							beforeSend : function(self, arg) {
								cola.widget("saveRoleComponentsButton").set(
										"disabled", true);
								cola.widget("saveRoleComponentsButton")
										.addClass("loading");
							},
							success : function() {
								cola.widget("saveRoleComponentsButton").set(
					...
					resetFalse : function() {
						if (this.get("value")) {
							if (model.get("pageNodeList")) {
								model.get("pageNodeList").each(
												function(pageNode) {
													if (pageNode.get("visible") == true) {
														pageNode.set("visible",
																false);
													}
													if (pageNode
															.get("editable") == true) {
...
//			});
		...
		function  resetNode(){
			var currentRole=model.get("roles").current;
			var entityList=currentRole.get("urls1");
			if(entityList){
				entityList.each(function(entity){
					try{
						var node=cola.widget("urlsTree").findNode(entity);
						if(node){
		...
		function getSelectNodes(nodes, urlIds, delUrlIds) {
			$.each(nodes, function() {
				var node = this;
				var urlNames, delNames;
				var checked = node.get('checked');
				var data = node.get('data');
				if (undefined == checked || checked) {
					urlIds.push(data.get("id"));
				}
				if (!checked) {
		...
		return model.widgetConfig({
			companysTree : {
				...
				currentNodeChange : function(self, arg) {
					...
					if (current) {
						...
						model.set("companyId", model.get("companyId"));
					 ...
					 * if(currentCompany){ var
					 * currentRoles=currentCompany.get("roles");
			...
			domTree : {
				...
				itemClick : function(self, arg) {
					self.get$Dom().find(".current-node").removeClass(
							"current-node");
